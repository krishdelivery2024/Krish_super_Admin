...
if(isset($_POST['type'])){
    if($_POST['type']=="orders"){
        ...
        if(isset($_POST['sub_type'])){
            ...
            if($_POST['sub_type']=='received'){
                $where .="o.active_status='received' AND (o.payment_method IN ('cod','wallet','LoyaltyPoints') OR o.payment_status='1')";
            ...
            }else if($_POST['sub_type']=='incomplete'){ 
                $where .="o.payment_method NOT IN ('cod','wallet','LoyaltyPoints') and o.payment_status='0' and o.active_status<>'cancelled'";
            }
        }else{
            $where="1"; 
        }

        // 🔹 Add seller_id condition if present
        if(!empty($seller_id)){
            if($where == "1"){
        ...
        if($where=="1"){
            $sql = "SELECT o.*,(SELECT name FROM users u WHERE u.id=o.user_id) as user_name 
                    FROM orders o 
                    WHERE o.payment_method IN ('cod','wallet','LoyaltyPoints') OR o.payment_status='1' 
        ...
        foreach($res as $row){
            ...
            $res[$i]['status'] = json_decode($res[$i]['status']);
                ...
                for($j=0; $j < count($res[$i]['items']); $j++){
                    ...
                    $item_qty         = !empty($res[$i]['items'][$j]['quantity']) ? (float) $res[$i]['items'][$j]['quantity'] : 0;
                    $item_vendor_price = !empty($res[$i]['items'][$j]['vendor_price']) ? (float) $res[$i]['items'][$j]['vendor_price'] : 0;
            
                    $vendor_total += ($item_qty * $item_vendor_price);
                }
                
                $res[$i]['total'] = number_format($vendor_total, 2, '.', '');
                $res[$i]['final_total'] = number_format($vendor_total, 2, '.', '');
            $i++;
        }
        $orders = $order = array();
	...
	$seller_id = (isset($_POST['seller_id'])) ? $db->escapeString($fn->xss_clean($_POST['seller_id'])) : "";
    if(!empty($seller_id)){
        $where = "o.seller_id = '$seller_id' AND";
    }else{
        $where ='';
    }
	$sql = "select *,(select name from users u where u.id=o.user_id) as user_name from orders o WHERE $where o.id=".$_POST['id'];
    $db->sql($sql);
    $res = $db->getResult();
    $i=0; $j=0;
    foreach($res as $row){
    ...
    }else if($_POST['type']=="store_settings") {
    $seller_id = (isset($_POST['seller_id'])) ? $db->escapeString($fn->xss_clean($_POST['seller_id'])) : "";
	$response = array();
	$sql_query = "SELECT * FROM seller WHERE id = '".$seller_id."' AND status = '1'";
	$db->sql($sql_query);
	$res=$db->getResult();
	$num = $db->numRows($res);
		if($num >0){

            $store_status = $res[0]['store_status'];
            
            
            
            if($store_status == 'true'){
                $update_store_status = 'false';
            }else{
                $update_store_status = 'true';
            }
            
                    
            $sql = 'UPDATE `seller` SET `store_status`="'.$update_store_status.'" WHERE `id`="'.$seller_id.'"';
		    $db->sql($sql);
    		...
    		$response['error']=false;
            $response['data'] = $res11;
    		$response['message'] = "Store status updated Successful!";
    				
	    }else{
			$response['error'] = true;
            $response['data'] = [];
			$response['message'] = "Invalid Seller Details!";
		}
