	...
	<?php 
		if(isset($_POST['btnAdd'])){
			...
			if(!empty($category_name) && !empty($category_subtitle) && empty($error['category_image'])){
				...
				$sql_query = "INSERT INTO category (name,main_cat,subtitle,image)
						VALUES('$category_name', '$main_category', '$category_subtitle', '$upload_image')";
					// Execute query
					$db->sql($sql_query);
					// store result 
					$result = $db->getResult();
...
<?php $db->disconnect(); ?>
	<script>
	    var uploadField = document.getElementById("category_image");
