			...
			if(!empty($name) && empty($error['image'])){
				...
				if(!empty($menu_image)){
					...
					$sql_query = "UPDATE category 
							SET name = '".$name."', main_cat ='".$main_category."',  subtitle = '".$subtitle."',image = '".$upload_image."'
							WHERE id = ".$ID;
					if($db->sql($sql_query)) {	
						// Execute query
						$db->sql($sql_query);
					...
					$sql_query = "UPDATE category 
							SET name = '".$name."', main_cat ='".$main_category."', subtitle = '".$subtitle."', image = '".$res[0]['image']."'
							WHERE id =".$ID;
						// Execute query
						$db->sql($sql_query);
						// store result 
					...
					<div class="form-group">
                      <label for="exampleInputEmail1">Main Category</label><?php echo isset($error['main_category_name']) ? $error['main_category_name'] : '';?>
                        <select class="form-control" id="main_category_name" name="main_category_name" required>
                            ...
                            <?php
                                $db->select('main_category','*',null);
                                $res1 = $db->getResult();
                                foreach($res1 as $category){
									$sel = '';
									if($res[0]['main_cat'] == $category['id']){$sel ="selected";}
