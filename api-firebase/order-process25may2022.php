...
//     $sql = "SELECT c.name FROM city c JOIN users u ON u.city=c.id where u.id=".$_POST['user_id'];
// 	$db->sql($sql);
...
//   $sql = "select latitude , longitude from `users` where id=".$user_id;
  ...
  $sql = "select latitude , longitude from `user_address` where id=".$address_id;
...
//  $address_id=6;
  ...
  if($delivery_method == 'delivery'){
    ...
    // print_r($settings);
// 	$store_lat = $settings['store_lattitude'] 13.030162630004416;
// 	$store_long = $settings['store_longitude'] 80.27051225327199;
    $store_lat = 13.023090;
	$store_long = 80.230520;
	$amt = $settings['delivery_charge'];
	$additional_amt = $settings['additional_delivery_charge'];

    $additional_amt=15;
    // print_r($additional_amt);
...
// 	print_r($distance);
	...
	echo json_encode($response);exit;
	            ...
	            $response['error'] = false;
                $response['delivery_charge'] = floatval(0);
                $response['package_weight'] = floatval('0.00');
            	$response['message'] = "Delivery charge";
            	echo json_encode($response);exit;   
  }
     $delivery_charges=0;
