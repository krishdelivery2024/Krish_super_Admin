...
if(isset($_POST['mobile']) && $_POST['mobile'] != '' && isset($_POST['password']) && $_POST['password'] != '') {
    ...
    if (!empty($mobile) && !empty($password)) {
        ...
        $sql_query = "SELECT *,(SELECT name FROM city a WHERE a.id=u.city) as city_name,(SELECT name FROM area c WHERE c.id=u.area) as area_name FROM `users` u WHERE `mobile` = '".$mobile."' AND `password` ='".$password."'";
        $db->sql($sql_query);
        $result=$db->getResult();
    ...
    $sql_query = "SELECT *,(SELECT name FROM area a WHERE a.id=u.area) as area_name,(SELECT name FROM city c WHERE c.id=u.city) as city_name FROM `users` u WHERE `id` = '".$_POST['user_id']."'";
        $db->sql($sql_query);
        $result=$db->getResult();
		if ($db->numRows($result) > 0) {
		    foreach($result as $row) {
