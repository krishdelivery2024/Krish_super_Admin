<?php
	...
	if(isset($_POST['accesskey']) && isset($_POST['product_id'])) {
		...
		$product_ids = json_decode($_POST['product_id']);
		
		if($access_key_received == $access_key){
			if(!empty($product_ids)){ 
			    $where="WHERE";
			    $j=count($product_ids)-1;
			    for($i=0;$i<count($product_ids);$i++){
			        ...
			        $where .=" id = ".$product_ids[$i]." OR";
			        }
			    }
                    $sql="SELECT *,(SELECT b.name FROM brand b WHERE p.brand_id=b.id) as brand_name FROM products p ".$where;
