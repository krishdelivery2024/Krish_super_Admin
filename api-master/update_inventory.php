<?php
	...
	if(isset($_POST['accesskey']) && isset($_POST['product_name'])) {
		...
		if($access_key_received == $access_key){
			...
			if(!empty($barcode)){ 
                $sql256="SELECT id, product_id FROM product_variant WHERE barcode_data = '".$barcode."' ";
                $db->sql($sql256);
                $res = $db->getResult();
               // print_r($res);
                    $sql="SELECT *,(SELECT b.name FROM brand b WHERE p.brand_id=b.id) as brand_name FROM products p WHERE id = '".$res[0]['product_id']."' ";
            ...
            foreach($res as $row){
                ...
                $row['ratings']=$ratings;
                $sql = "SELECT *,(SELECT short_code FROM unit u WHERE u.id=pv.measurement_unit_id) as measurement_unit_name,(SELECT short_code FROM unit u WHERE u.id=pv.stock_unit_id) as stock_unit_name FROM product_variant pv WHERE pv.product_id=".$row['id']." AND pv.barcode_data = '".$barcode."' ";
                $db->sql($sql);
