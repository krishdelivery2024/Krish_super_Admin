...
<?php
$sql = "SELECT * FROM `category`  WHERE main_cat='$main_cat_id' ORDER BY `id` DESC";
$db->sql($sql);
...
if(isset($_GET['category_id']) && isset($_GET['subcategory_id'])){ 
    ...
    $sql = "SELECT * FROM `products` where `seller_id` = '".$seller_id."' AND `category_id` = '".$_GET['category_id']."' AND `subcategory_id`='".$_GET['subcategory_id']."'ORDER BY `row_order` ASC";
    $db->sql($sql);
    $res=$db->getResult();
}
    if((isset($_GET['category_id'])&&($_GET['category_id']!=='')) && isset($_GET['subcategory_id'])&&($_GET['subcategory_id']=='')){ 
        ...
        $category_id = $db->escapeString($_GET['category_id']);
        $sql = "SELECT * FROM `products` where `seller_id` = '".$seller_id."' AND `category_id` = '".$category_id."' ORDER BY `row_order` ASC";
    $db->sql($sql);
     $res = $db->getResult();
    }
   if((isset($_GET['category_id'])&&($_GET['category_id']=='')) && isset($_GET['subcategory_id'])&&($_GET['subcategory_id']=='')){  
        $sql = "SELECT * FROM `products` WHERE `seller_id` = '".$seller_id."' ORDER BY `row_order` ASC";
        $db->sql($sql);
        $res = $db->getResult();
    }
    if((isset($_GET['category_id'])&&($_GET['category_id']=='')) && isset($_GET['subcategory_id'])&&($_GET['subcategory_id']!=='')){
        $subcategory_id=$_GET['subcategory_id'];
        $sql = "SELECT * FROM `products` where `seller_id` = '".$seller_id."' AND `subcategory_id` = '".$subcategory_id."' ORDER BY `row_order` ASC";
        $db->sql($sql);
        $res = $db->getResult();
    }
     if((!isset($_GET['category_id']))&& !(isset($_GET['subcategory_id'])&&($_GET['subcategory_id']==''))){ 
        $sql = "SELECT * FROM `products` WHERE `seller_id` = '".$seller_id."' ORDER BY `row_order` ASC";
        $db->sql($sql);
        $res = $db->getResult();
    }
                        ...
                        <div class="form-group" style="overflow-y:scroll;height:400px;">
                            ...
                            <li id=<?php echo $product["id"]; ?>>
                                <?php if(!empty($product["image"])){
                                    echo "&nbsp;<img src='$product[image]' style='height:30px !important'> ".$product["name"];
                                }else{ 
                                    echo "&nbsp;<img src='images/logo.png' style='height:30px !important' > ".$product["name"];
                                } ?>
                            </li>
                            <?php } ?>
                            </ol>
